    ...
    <div class="page-content">
      ...
      <div class="filter-bar">
        <div class="filter-group">
          ...
          <div class="filter-item">
            <label>维修类型：</label>
            <select v-model="filterRepairType" class="filter-select">
          ...
          <div class="card-content" @click="viewDetail(demand.id)">
            ...
            <div class="demand-info">
              <div class="info-row">
                ...
                <span class="info-value">{{ demand.vesselName }}</span>
              </div>
              <div class="info-row">
                <span class="info-label">维修内容：</span>
                <span class="info-value">{{ getRepairTypeLabel(demand.repairType) }}</span>
              </div>
              <div class="info-row">
                <span class="info-label">紧急程度：</span>
                <span class="info-value" :class="`urgency-${demand.urgency}`">{{ getUrgencyLabel(demand.urgency) }}</span>
              </div>
              <div class="info-row">
                <span class="info-label">维修地点：</span>
                <span class="info-value">{{ getRepairLocationLabel(demand.repairLocation) }}</span>
              </div>
            </div>
      ...
      <div class="modal-content large">
        ...
        <div class="modal-body-wrapper">
          <form @submit.prevent="saveEdit" class="modal-form">
            ...
            <div class="form-section">
              ...
              <div class="form-row">
                <div class="form-item required full-width">
                  <label>需求标题</label>
                  <input v-model="editForm.title" type="text" class="form-input" placeholder="请输入需求标题（不超过30字）" maxlength="30" required>
                </div>
              </div>
              ...
              <div class="form-row">
                ...
                <div class="form-item required">
                  <label>联系人</label>
                  <input v-model="editForm.contact" type="text" class="form-input" placeholder="请输入联系人" required>
                </div>
              </div>
              ...
              <div class="form-row">
                <div class="form-item required">
                  <label>联系电话</label>
                  <input v-model="editForm.phone" type="tel" class="form-input" placeholder="请输入联系电话" required>
                </div>
            ...
            <!-- 维修需求 -->
            <div class="form-section">
              <h3 class="section-title">维修需求</h3>
              
              <div class="form-row">
                <div class="form-item required full-width">
                  <label>关联自有船舶</label>
                  <select v-model="editForm.vesselId" class="form-input" required>
              ...
              <div class="form-row">
                <div class="form-item required">
                  <label>维修内容</label>
                  <select v-model="editForm.repairType" class="form-input" required>
                    <option value="">请选择</option>
                    <option value="routine">常规保养</option>
                    <option value="fault">故障维修</option>
                    <option value="major">大修</option>
                ...
                <div class="form-item required">
                  <label>维修紧急程度</label>
                  <select v-model="editForm.urgency" class="form-input" required>
                    <option value="">请选择</option>
                    <option value="normal">普通</option>
                    <option value="urgent">紧急</option>
                    <option value="express">加急</option>
              ...
              <div class="form-row">
                <div class="form-item required full-width">
                  <label>故障/维修部位</label>
                  <textarea v-model="editForm.faultPart" class="form-textarea" placeholder="请详细描述故障部位和具体问题" rows="3" required></textarea>
                </div>
              </div>
              ...
              <div class="form-row">
                <div class="form-item required">
                  <label>维修地点偏好</label>
                  <select v-model="editForm.repairLocation" class="form-input" required>
                    <option value="">请选择</option>
                ...
                <div class="form-item">
                  ...
                  <div class="input-group">
                    <input v-model.number="editForm.budgetMin" type="number" class="form-input" placeholder="最小">
                    <span class="input-separator">-</span>
                    <input v-model.number="editForm.budgetMax" type="number" class="form-input" placeholder="最大">
    ...
    <div v-if="toggleConfirmVisible" class="modal-overlay" @click.self="toggleConfirmVisible = false">
      <div class="modal-content small">
        ...
        <div class="modal-body">
          <p>确定要{{ toggleAction === 'offline' ? '下架' : '上架' }}该维修需求吗？</p>
        </div>
        <div class="modal-footer">
...
const demands = ref([
  {
    ...
    urgency: 'express',
    faultPart: '主机喷油泵故障，需要紧急更换',
    repairLocation: 'port',
    budgetMin: 20,
    budgetMax: 50,
    contact: '赵船长',
    phone: '13600136000',
    expectDate: '2026-02-05',
    status: 'approved',
  ...
  },
  {
    ...
    urgency: 'normal',
    faultPart: '全船年度维护保养',
    repairLocation: 'shipyard',
    budgetMin: 80,
    budgetMax: 120,
    contact: '刘经理',
    phone: '13500135000',
    expectDate: '2026-03-15',
    status: 'approved',
...
const filteredDemands = computed(() => {
  return demands.value.filter(demand => {
    ...
    if (filterRepairType.value && demand.repairType !== filterRepairType.value) return false
    if (filterUrgency.value && demand.urgency !== filterUrgency.value) return false
...
// 获取维修地点标签
const getRepairLocationLabel = (location) => {
  const labels = {
    shipyard: '船厂维修',
    onsite: '上门维修',
    port: '港口就近维修'
...
// 查看详情
const viewDetail = (id) => {
  console.log('[v0] 查看维修需求详情:', id)
}

// 编辑需求
const editDemand = (id) => {
  console.log('[v0] 编辑维修需求:', id)
  ...
  editForm.value = {
    ...
    vesselId: demand.vesselId,
    repairType: demand.repairType,
    urgency: demand.urgency,
    faultPart: demand.faultPart,
    repairLocation: demand.repairLocation,
...
// 跳转到发布页面
const goToPublish = () => {
  router.push('/ship-repair/publish-demand/repair')
}
